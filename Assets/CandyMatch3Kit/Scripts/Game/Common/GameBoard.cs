...
namespace GameVanilla.Game.Common
{
    ...
    public class GameBoard : MonoBehaviour
    {
        ...
        private RuntimePlayerGerm playerGerm = new RuntimePlayerGerm();
        private RuntimeUseItem useItem = new RuntimeUseItem();
        private RuntimePurchaseQuest purchaseQuest = new RuntimePurchaseQuest();
        private List<RuntimeGameQuest> runtimeGameQuests = new List<RuntimeGameQuest>();
        ...
        private readonly MatchDetector twobytwoShapedMatchDetector = new TwobytwoMatchDetector();
        ...
        private void Start()
        {
            ...
            runtimeGameQuests.Add(playerGerm);
            runtimeGameQuests.Add(useItem);
            runtimeGameQuests.Add(purchaseQuest);
            foreach(var runtimeGameQuest in runtimeGameQuests)
            {
                runtimeGameQuest.GameStart();
            }
        }
        ...
        public void HandleInput()
        {
            ...
            if (drag && selectedTile != null) // drag가 true , 타일이 선택이되었다.
            {
                var hit = Physics2D.Raycast(Camera.main.ScreenToWorldPoint(Input.mousePosition), Vector2.zero);
                if (hit.collider != null && hit.collider.gameObject != selectedTile)
                {
                    ...
                    else
                    {
                        ...
                        var tileB = selectedTile;
                        if (!(tileA.GetComponent<Tile>().x != tileB.GetComponent<Tile>().x &&
                              tileA.GetComponent<Tile>().y != tileB.GetComponent<Tile>().y) || (tileA.GetComponent<Tile>().x != tileB.GetComponent<Tile>().x &&
        ...
        public void HandleSwitchBoosterInput(BuyBoosterButton button)
        {
            ...
            if (drag && selectedTile != null)
            {
                ...
                if (hit.collider != null && hit.collider.gameObject != selectedTile)
                {
                    ...
                    RuntimeUseItemIncrease("BoosterSwitch");
                    Debug.Log("switch");
        ...
        public void ExplodeTile(GameObject tile, bool didAnySpecialCandyExplode = false)
        {
            ...
            foreach (var explodedTile in explodedTiles)
            {
                ...
                if (idx != -1)
                {
                    ...
                    RuntimeQuestGermIncrese(explodedTile);
        ...
        {
            return HasHorizontalMatch(x, y) || HasVerticalMatch(x, y) || HasTwobytwoMath(x,y);
        }
        ...
        private bool HasTwobytwoMath(int x , int y)
        {
            var tile = GetTile(x, y);

            int twoRectangleLength = 0;            

            if(tile.GetComponent<Candy>() != null)     // (x , y ) , (x , y - 1) , (x+1 , y) , (x+1 , y-1) 
            {
                if(y-1 >= 0 && x+1 <level.width && GetTile(x, y-1) != null && GetTile(x, y-1).GetComponent<Candy>() != null &&
                    GetTile(x, y-1).GetComponent<Candy>().color == tile.GetComponent<Candy>().color &&
                    GetTile(x+1, y) != null && GetTile(x+1, y).GetComponent<Candy>() != null &&
                    GetTile(x+1, y).GetComponent<Candy>().color == tile.GetComponent<Candy>().color &&
                    GetTile(x+1, y - 1) != null && GetTile(x+1, y - 1).GetComponent<Candy>() != null &&
                    GetTile(x+1, y - 1).GetComponent<Candy>().color == tile.GetComponent<Candy>().color)
                {
                    twoRectangleLength++;                   
                }

                if (y - 1 >= 0 && x -1 >= 0 && GetTile(x, y - 1) != null && GetTile(x, y - 1).GetComponent<Candy>() != null &&
                   GetTile(x, y - 1).GetComponent<Candy>().color == tile.GetComponent<Candy>().color &&
                   GetTile(x - 1, y) != null && GetTile(x - 1, y).GetComponent<Candy>() != null &&
                   GetTile(x - 1, y).GetComponent<Candy>().color == tile.GetComponent<Candy>().color &&
                   GetTile(x - 1, y - 1) != null && GetTile(x - 1, y - 1).GetComponent<Candy>() != null &&
                ...
                if ( y+1 < level.height && x -1 >= 0 &&GetTile(x, y + 1) != null && GetTile(x, y + 1).GetComponent<Candy>() != null &&
                  GetTile(x, y + 1).GetComponent<Candy>().color == tile.GetComponent<Candy>().color &&
                  GetTile(x - 1, y) != null && GetTile(x - 1, y).GetComponent<Candy>() != null &&
                  GetTile(x - 1, y).GetComponent<Candy>().color == tile.GetComponent<Candy>().color &&
                  GetTile(x - 1, y + 1) != null && GetTile(x - 1, y + 1).GetComponent<Candy>() != null &&
                ...
                if (y + 1 < level.height && x + 1 < level.width && GetTile(x, y + 1) != null && GetTile(x, y + 1).GetComponent<Candy>() != null &&
                  GetTile(x, y + 1).GetComponent<Candy>().color == tile.GetComponent<Candy>().color &&
                  GetTile(x + 1, y) != null && GetTile(x + 1, y).GetComponent<Candy>() != null &&
                  GetTile(x + 1, y).GetComponent<Candy>().color == tile.GetComponent<Candy>().color &&
                  GetTile(x + 1, y + 1) != null && GetTile(x + 1, y + 1).GetComponent<Candy>() != null &&
            ...
            for (var j = 0; j < level.height; j++)
            {
                for (var i = 0; i < level.width; i++)
                {
                    ...
                    if (tile != null)
                    {
                        ...
                        if (i < level.width - 1 && j < level.height - 1 )
                        {
                            var other = GetTile(i + 1, j + 1);
                           

                            if (other != null)
                            {
                                SetTile(other, i, j);
                                SetTile(tile, i + 1, j + 1);

                                if (HasMatch(i, j) || HasMatch(i + 1, j + 1))
                                {
                                    var swap = new Swap { tileA = tile, tileB = other };
                                    swaps.Add(swap);
                                }
                            ...
                            SetTile(tile, i, j);
                            SetTile(other, i + 1, j + 1);
                           
                        }
                        if (i - 1 >= 0 &&  j < level.height-1)
                        {
                            var other = GetTile(i - 1, j + 1);
        ...
        private bool HandleMatches(bool isPlayerMatch)
        {
            ...
            var tbtMatches = twobytwoShapedMatchDetector.DetectMatches(this);

            if (tbtMatches.Count > 0)
            {
                matches.AddRange(tbtMatches);
                foreach (var match in horizontalMatches)
                {
                    var found = false;
                    foreach (var match2 in tbtMatches)
                    {
            ...
            else if (tShapedMatches.Count > 0)
            {
        ...
        private List<GameObject> GetTilesToHighlight(GameObject tile, int x, int y)
        {
            ...
            else if (HasTwobytwoMath(x, y))
            {
                ...
                if (y - 1 >= 0 && x + 1 < level.width && GetTile(x, y - 1) != null && GetTile(x, y - 1).GetComponent<Candy>() != null &&
                    ...
                    GetTile(x + 1, y - 1).GetComponent<Candy>().color == tile.GetComponent<Candy>().color)
                {
                    if (!tilesToHighlight.Contains(GetTile(x, y - 1)))
                    {
                        tilesToHighlight.Add(GetTile(x, y - 1));
                    }


                    if (!tilesToHighlight.Contains(GetTile(x+1, y )))
                    {
                        tilesToHighlight.Add(GetTile(x+1, y));
                    }

                    if (!tilesToHighlight.Contains(GetTile(x+1, y - 1)))
                    {
                        tilesToHighlight.Add(GetTile(x+1, y - 1));
                    }
        ...
        private IEnumerator AwardSpecialCandiesAsync()
        {
            ...
            for(int i =0; i< tiles.Count; i++)
            {
                ...
                if (tiles[i] != null && IsSpecialCandy(tiles[i]))
                {
                    ExplodeTile(tiles[i]);
                    ApplyGravity();
        ...
        private void RuntimeQuestGermIncrese(GameObject candy)
        {
            if(playerGerm.runtimeGerm_action == null)
            {
                return;
            }


            if(candy.GetComponent<Candy>() != null)
            {
                var germ = candy.GetComponent<Candy>();
                if(germ.color == CandyColor.Red)
                {
                    playerGerm.runtimeGerm_action("red");
                }
                else if (germ.color == CandyColor.Yellow)
                {
                    playerGerm.runtimeGerm_action("yellow");
                }
                else if (germ.color == CandyColor.Purple)
                {
                    playerGerm.runtimeGerm_action("purple");
                }
                else if (germ.color == CandyColor.Green)
                {
                    playerGerm.runtimeGerm_action("green");
                }
                else if (germ.color == CandyColor.Blue)
                {
                    playerGerm.runtimeGerm_action("blue");
                }
                else if (germ.color == CandyColor.Orange)
                {
                    playerGerm.runtimeGerm_action("orange");
                }
        ...
        public void RuntimeUseItemIncrease(string boostername)
        {
            if (useItem.runtimeGerm_action == null)
            {
                return;
            }
            ...
            useItem.runtimeGerm_action(boostername);


        }

        public void RuntimePurchaseIncrease(BoosterType boosterType)
        {
            if (purchaseQuest.runtimeGerm_action == null)
            {
                return;
            }
            ...
            {
                case BoosterType.Lollipop:
                    purchaseQuest.runtimeGerm_action("BoosterLollipop");
                    break;

                case BoosterType.Bomb:
                    purchaseQuest.runtimeGerm_action("BoosterBomb");
                    break;

                case BoosterType.Switch:
                    purchaseQuest.runtimeGerm_action("BoosterSwitch");
                    break;

                case BoosterType.ColorBomb:
                    purchaseQuest.runtimeGerm_action("BoosterColorBomb");
                    break;
            }
